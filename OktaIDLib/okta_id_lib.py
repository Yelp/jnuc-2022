...
class OktaIDLib():
    ...
    def _execute_request(self, api_url):
        response = requests.request("GET", api_url, headers=self._default_headers, data={})
    ...
    def get_ad_groups(self, user_id):
        ...
        return [g['profile']['dn'] for g in groups if g['type'] == 'APP_GROUP' and g['profile'].get('dn') is not None]
